...
if(FORCE_HTTPS && location.protocol==='http:')
  ...
  navigator.serviceWorker.register('/RomPatcher/_cache_service_worker.js', {scope: '/RomPatcher/'});
...
try{
  webWorkerApply=new Worker('./src/scripts/worker_apply.js');
  ...
  webWorkerCreate=new Worker('./src/scripts/worker_create.js');
  ...
  webWorkerCrc=new Worker('./src/scripts/worker_crc.js');
...
addEvent(window,'load',function(){
  ...
  if(CAN_USE_WEB_WORKERS){
    ...
    zip.workerScriptsPath='./src/scripts/zip.js/';
  }else{
    zip.useWebWorkers=false;

    var script=document.createElement('script');
    script.src='./src/scripts/zip.js/inflate.js';
